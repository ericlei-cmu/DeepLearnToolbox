%I think this is just a sanity checking function, nothing else calls it

function dcnnnumgradcheck(net, x, y)
    epsilon = 1e-4;
    er      = 1e-8;
    n = numel(net.layers);
    for j = 1 : numel(net.ffb)
        ...
        net_m = dcnnff(net_m, x); net_m = dcnnbp(net_m, y);
        net_p = dcnnff(net_p, x); net_p = dcnnbp(net_p, y);
